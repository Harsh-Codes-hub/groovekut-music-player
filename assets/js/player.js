...
function applyBgColor(r, g, b) {
  if (playerBg) {
    playerBg.style.background = `radial-gradient(ellipse at 30% 40%, rgba(${r},${g},${b},0.35) 0%, transparent 65%),
...
async function buildQueue() {
  ...
  try {
    ...
    if (data.success && data.queue.length) {
      const currentId = (state.queue[state.queueIndex] || INITIAL_SONG).id;
      state.queue = data.queue;
      state.shuffleOrder = generateShuffleOrder(data.queue.length);
      // Force current song to front of shuffle order
      const pos = state.shuffleOrder.indexOf(state.queueIndex);
      state.shuffleOrder.splice(pos, 1);
      state.shuffleOrder.unshift(state.queueIndex);

      // Keep current song at its position in new queue
      const newIndex = data.queue.findIndex((s) => s.id == currentId);
      state.queueIndex = newIndex !== -1 ? newIndex : 0;

      renderQueue();
      saveQueueToSession();

      // Scroll active item into view in queue panel
      setTimeout(() => {
...
function renderQueue() {
  queueList.innerHTML = "";
  const displayOrder = state.shuffle
    ? state.shuffleOrder
  ...
  displayOrder.forEach((songIdx, displayIdx) => {
    ...
    item.className =
      "queue-item" + (songIdx === state.queueIndex ? " active" : "");
...
function loadSong(song) {
  ...
  moodEl.textContent = song.mood_tag
    ? song.mood_tag.charAt(0).toUpperCase() + song.mood_tag.slice(1)
    : "";
...
function getPrevIndex() {
  if (state.shuffle) {
    const pos = state.shuffleOrder.indexOf(state.queueIndex);
    const prev =
      (pos - 1 + state.shuffleOrder.length) % state.shuffleOrder.length;
...
if (likeBtn) {
  likeBtn.addEventListener("click", async () => {
    ...
    const res = await fetch("/groovekut/api/like.php", {
      method: "POST",
      body: fd,
    });
    const data = await res.json();
    if (data.success) {
      ...
      likeBtn.querySelector("i").className = data.liked
        ? "ri-heart-3-fill"
        : "ri-heart-3-line";
    }
  });
...
function updateNowPlaying(song) {
  ...
  indicator.onclick = () =>
    (window.location.href = `/groovekut/player.php?id=${song.id}`);
